    ...
    <Table className="status-table">
      ...
      <tbody>
        <tr>
          ...
          {cellCompleted(status.answers_extracted)}
        </tr>
        <tr>
          <td>Question Generation</td>
          {cellCompleted(status.questions_generated)}
        </tr>
        <tr>
          <td>Answer Generation</td>
          {cellCompleted(status.answers_generated)}
        </tr>
        <tr>
          <td>Question Filtering</td>
          {cellCompleted(status.questions_filtered)}
        </tr>
      </tbody>
    </Table>
  );
...
function QueueNumber(props) {
  ...
  if (props.queue_number === 1) {
    return (
      <div>
        <div>
          Your document is being processed. The status will auto-refresh every 5
          seconds, It typically takes about 3-6 seconds per paragraph.
        </div>
        <br />
    ...
    return (
      <div>
        <div>
          Your document is in the queue, {props.queue_number - 1} document(s)
          before you. The status will auto-refresh every 5 seconds. It typically
          takes 10-15 seconds per document.
        </div>
        <br />
        <div>{status_table}</div>
      </div>
...
class SquashDemo extends React.Component {
  ...
  toggleSpecific(para_index, qa_index) {
    ...
    new_expanded[para_index][qa_index] = !this.state.expanded[para_index][
      qa_index
    ];
    this.setState({
      ...
      <div className="container-fluid">
        ...
        <Row>
          <Col md={{ order: 2, size: 5 }} xs={{ order: 1 }}>
            <h5>
              A demo for{" "}
              <a href="https://arxiv.org/abs/1906.02622">
                Generating Question-Answer Hierarchies
              </a>
            </h5>
            <p>
              This system converts a sequence of paragraphs into a hierarchy of
              question-answer pairs, with general questions at the top level and
              specific questions underneath. Check out our{" "}
              <a href="http://squash.cs.umass.edu/">landing page</a> for more
              details. Feel free to fork and use the{" "}
              <a href="https://github.com/martiansideofthemoon/squash-website">
                source code
              </a>{" "}
              for this demo.
            </p>
            <p>
              Contact{" "}
              <a href="mailto:kalpesh@cs.umass.edu">kalpesh@cs.umass.edu</a> if
            ...
            {squash_loaded && (
              <div className="switch-answer-div">
                ...
                <div className="toggle-div">
                  ...
                  <div className="toggle-sub-div">
                    <Toggle
                      defaultChecked={this.state.ans_mode === "predicted"}
                      icons={false}
                      onChange={() => this.toggleAnswerMode()}
                    />
                  </div>
                  <div className="toggle-sub-div">
                ...
                <div>
                  <FormText>
                    When set to "Original answers", the displayed answers will
                    be the same as the text spans provided to the question
                    generation system as input (see section 3.1 of the{" "}
                    <a href="https://arxiv.org/pdf/1906.02622.pdf#page=3">
                      paper
                    </a>
                    ). When set to "Predicted answers", the displayed answers
                    are the output of a BERT-based QA model when fed the
                    generated questions. Predicted answers are generally shorter
                    and more accurate, while original answers offer more insight
                    into the context that produced the question.{" "}
                  </FormText>
                </div>
                <hr />
              </div>
            ...
            <div>
              <div className="precomputed-div">
                ...
                >
                  ...
                  <DropdownMenu>
                    <DropdownItem tag="a" href="/?id=04bf8a42f934944809e76ec1">
                      Cricket
                    </DropdownItem>
                    <DropdownItem tag="a" href="/?id=bbd2ba3e440ff9abb52f211f">
                      The Fellowship of the Ring
                    </DropdownItem>
                    <DropdownItem tag="a" href="/?id=e3aebbb37fa9d1c48638aa46">
                      Neil Armstrong
                    </DropdownItem>
                  </DropdownMenu>
                </Dropdown>
          ...
          <Col md={{ order: 2, size: 7 }} xs={{ order: 1 }}>
            {squash_loaded && (
              <SquashForest
                ...
                toggleSpecific={(para_index, qa_index) =>
                  this.toggleSpecific(para_index, qa_index)
                }
                expanded={this.state.expanded}
              />
            )}
            {this.state.queue_number !== null &&
              this.state.queue_number !== 0 && (
                <QueueNumber
                  queue_number={this.state.queue_number}
